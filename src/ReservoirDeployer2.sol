...
contract ReservoirDeployer2 {
    using FactoryStoreLib for GenericFactory;

    // Steps.
    uint256 public constant TERMINAL_STEP = 5;
    ...
    bytes32 public constant ROUTER_HASH = bytes32(0x262458524d9c8928fe7fd7661236b93f6d6a9535182f48fd582a75f18bfbf85f);

    // Deployment addresses.
    GenericFactory public factory;
    OracleCaller public oracleCaller;
    ReservoirRouter public router;
    ...
    // requires that the caller provide a good implementation of WETH. Else we're screwed
    function deployRouter(bytes memory aRouterBytecode, address aWETH) external returns (ReservoirRouter) {
        require(step == 4, "OC_STEP: OUT_OF_ORDER");
        require(keccak256(aRouterBytecode) == ROUTER_HASH);

        address payable lRouterAddress;

        bytes memory lInitCode;
        uint256 lFreeMem = aRouterBytecode.length;
...
//        }
        ...
        // SAFETY:
        address lFactory = address(factory);
        assembly ("memory-safe") {

            mstore(lFreeMem, lFactory)
            mstore(add(lFreeMem, 0x20), aWETH)
            lRouterAddress :=
                create(
                    0,
                    add(aRouterBytecode, 0x20),
                    mload(aRouterBytecode)
                )
        }
        require(lRouterAddress != address(0), "RR_STEP: DEPLOYMENT_FAILED");

        router = ReservoirRouter(lRouterAddress);
        step += 1;
        return router;
    }
